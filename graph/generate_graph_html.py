...
graphData.edges.forEach(e => {
  if (outRel[e.from]) outRel[e.from].push({ label: e.label, other: e.to });
  if (inRel[e.to]) inRel[e.to].push({ label: e.label, other: e.from });
});
...
function renderPanel(d) {
  ...
  if (outs.length) {
    ...
    outs.forEach(r => { html += '<div class="rel-item"><span class="arrow">—' + r.label + '→</span> ' + r.other + '</div>'; });
    html += '</div></div>';
  }
  if (ins.length) {
    html += '<div class="field"><div class="field-key">入边关系</div><div class="rel-list">';
    ins.forEach(r => { html += '<div class="rel-item">' + r.other + ' <span class="arrow">—' + r.label + '→</span></div>'; });
    html += '</div></div>';
  }
